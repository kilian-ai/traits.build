...
- `local/tunnel-listen.sh` now runs `websocat` with `-E` (`--exit-on-eof`) so each localhost client gets a clean tunnel stream. Without this, sequential SSH/SFTP attempts can inherit stale stream state and surface `Bad packet length` / `message authentication code incorrect`.
