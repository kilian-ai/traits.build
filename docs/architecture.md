...
- The Apptron shell bootstrap now prefers `wss://relay.traits.build/linux/tunnel` by default (unless `network` query param overrides it), normalizes legacy relay values (`wss://relay.traits.build/x/net`) to the tunnel endpoint, probes WebSocket reachability, and falls back to `wss://apptron.dev/x/net` when relay is unavailable.
