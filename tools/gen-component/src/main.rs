...
fn render_crate(
    ...
    let ns = dotted.split('.').next().unwrap_or("traits").to_string();
...
fn render_wit(pkg_name: &str, kebab: &str, dotted: &str, t: &TraitToml) -> String {
    ...
    out.push_str(&format!("interface {kebab} {{\n"));

    let sig = t.signature.as_ref().expect("signature checked earlier");
    out.push_str(&format!("  {kebab}: func("));
...
fn json_push_for(ty: &TraitType, name: &str) -> String {
    match ty {
        TraitType::Int => format!("args.push(serde_json::json!({name}));"),
        TraitType::Float => format!("args.push(serde_json::json!({name}));"),
        TraitType::Bool => format!("args.push(serde_json::json!({name}));"),
        TraitType::String => {
            format!("args.push(serde_json::Value::String({name}.to_string()));")
        }
        TraitType::Bytes => format!("args.push(serde_json::json!({name}));"),
        TraitType::Null => format!("args.push(serde_json::Value::Null); let _ = {name};"),
        TraitType::Handle => format!("args.push(serde_json::json!({name}));"),
        TraitType::Any => format!(
            "args.push(serde_json::from_str(&{name}).unwrap_or_else(|_| serde_json::Value::String({name}.to_string())));"
        ),
        TraitType::List(_) => format!("args.push(serde_json::json!({name}));"),
        TraitType::Map(_, _) => format!(
            "args.push(serde_json::Value::Object({name}.iter().cloned().map(|(k,v)| (k, serde_json::Value::String(v))).collect()));"
        ),
