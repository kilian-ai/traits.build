...
- **Apptron IDE must retry wanix port handoff.** In `apptron-ide/index.html`, after bounded hidden-shell startup wait, `runtime.createPort()` can still fail transiently. The message-port handoff must retry (for example 30s window) before failing, otherwise Explorer/terminal can stay stuck waiting for a never-delivered `wanix` port.
