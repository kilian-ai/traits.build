...
- **sys.shell WASM PVFS sync:** `sys.shell` now loads `traits.pvfs` into its dedicated internal `CliSession` before each command and writes `session.vfs_dump()` back to `traits.pvfs` after execution. This keeps tool-driven edits visible in the primary terminal session (`cli_input` refresh path).
