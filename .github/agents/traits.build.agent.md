...
- **Apptron IDE hidden bridge runtime should not allocate xterm.** In `static/apptron/index.html`, when `ide_bridge=1` is present, boot Wanix runtime only and clear stale terminal-id hints (`vm/1/fsys/tmp/.apptron-terminal-id`, `localStorage['apptron-terminal-id']`). Let the extension fully own PTY allocation/wiring to avoid split ownership of `web/dom/*` streams.
