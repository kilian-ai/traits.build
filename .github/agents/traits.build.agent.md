...
- **Lua interactive stdin in terminal:** when a WASM CLI `lua <file.lua>` script uses `io.read` and no stdin args are provided, the CLI now emits an interactive Lua sentinel and terminal.js captures stdin lines directly from terminal input (`lua stdin> ` prompt, Ctrl+D to finish), then re-runs the Lua script with captured `input.stdin`.
