...
- **Apptron IDE publishing via Service Worker.** `publish-sw.js` at repo root registers a SW with root scope. The IDE page (`apptron-ide/index.html`) auto-syncs files from `vm/1/fsys/public` to the SW Cache API every 5 seconds with 2s timeouts per VFS operation. Files in guest `/public/hello.txt` become accessible at `https://www.traits.build/hello.txt`. Reserved paths (`/`, `/index.html`, `/static/*`, `/traits/*`, `/relay/*`) pass through to the network so the SPA and API are unaffected. The SW supports `publish-file`, `unpublish-file`, `clear-published`, and `list-published` message types. Recursive directory walking (depth 3) handles nested published directories.
- **Apptron IDE `/public` must be auto-created at boot.** Because the IDE boots the hidden shell without `with_env=1` (project-fs disabled to avoid the boot.go:597 fatal), `/public` does not exist in the default Wanix namespace and `readDir('vm/1/fsys/public')` throws `"open vm/1/fsys/public: file does not exist"`. The publish sync loop silently returns `[]` and nothing gets cached in the SW. Fix: the IDE page calls `runtime.makeDir('vm/1/fsys/public')` (idempotent, 2s timeout) once after `window.apptron.ready` resolves and before starting the 5s sync interval. Files written to `/public` are memfs-only for the browser session (reload clears them) but still get published through the SW cache for the lifetime of the tab.
