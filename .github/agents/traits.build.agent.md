...
- **Apptron IDE hidden bridge runtime should create xterm but skip `#console/data` bind.** In `static/apptron/index.html`, when `ide_bridge=1` is present, create `web/dom/new/xterm` (to boot shell), skip DOM append and `bind #console/data ...`, persist terminal id to both `vm/1/fsys/tmp/.apptron-terminal-id` and `localStorage['apptron-terminal-id']` for extension handoff, and still inject the relay bootstrap rc file into the hidden shell so IDE terminal sessions inherit WANIX network env.
- **Apptron IDE terminal should prefer direct xterm channel.** In `system/src/web/extension.ts`, resolve terminal id from `vm/1/fsys/tmp/.apptron-terminal-id` first, then `localStorage['apptron-terminal-id']`, then same-origin iframe `window.apptron.terminalId`, and use `web/dom/<id>/data` for read/write; only fall back to `#console/data` when all id sources are unavailable.
...
- **Apptron IDE terminal should prefer extension-owned `#console/data` binding.** In `system/src/web/extension.ts`, allocate `web/dom/new/xterm`, bind `task/1/ctl` `#console/data` -> `web/dom/<id>/data`, and open PTY streams on `#console/data` to avoid hidden-iframe timing races.
