...
- **kernel.cli userland tool surface:** semantic builtins now include `echo`, `pwd`, `cat`, `head`, `tail`, `grep`, `wc`, `sed` (`s/.../.../[g]` subset), `test`/`[ ]`, `find` (with `-name`, `-type`, `-maxdepth`), and `curl` (mapped through `sys.call`). Extend this set in `cli.rs` before adding terminal-side hacks.
