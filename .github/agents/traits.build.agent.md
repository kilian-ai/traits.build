...
- **Apptron shell should prefer default relay with fallback.** In `static/apptron/index.html`, default network bootstrap should probe `wss://relay.traits.build/x/net` first, then fall back to origin-derived `/x/net`, while preserving explicit `network` query overrides.
