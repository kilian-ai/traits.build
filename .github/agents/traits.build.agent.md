...
- **Apptron IDE hidden bridge runtime should create xterm but skip `#console/data` bind.** In `static/apptron/index.html`, when `ide_bridge=1` is present, create `web/dom/new/xterm` (to boot shell), skip DOM append and `bind #console/data ...`, and write terminal id to `vm/1/fsys/tmp/.apptron-terminal-id` for extension handoff.
- **Apptron IDE terminal should prefer direct xterm channel.** In `system/src/web/extension.ts`, resolve terminal id from `vm/1/fsys/tmp/.apptron-terminal-id` (fallbacks allowed) and use `web/dom/<id>/data` for read/write; if the id file is missing, also scan same-origin iframes for `window.apptron.terminalId` before falling back to `#console/data`.
