...
- **Apptron IDE PTY probe sequencing:** current debug marker is `pty-bridge-selftest-20260421-05`. Startup logs should include `bridge self-test: ...` (producer-side hidden-runtime ingress check) before PTY attach, then `post-first-chunk: entering readiness probe ...` followed by `writing readiness probe ...` and either `readiness probe succeeded ...` or timeout/fallback diagnostics. Missing these lines usually indicates stale extension assets are still cached.
