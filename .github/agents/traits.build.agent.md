...
- **kernel.cli unknown-command edit strategy:** for file/code edits, fallback prompt now prefers full-file rewrite commands (`write`/`tee` + final `cat` verification) and explicitly avoids brittle `sed -i` and chained `echo >>` edit patterns.
