...
- **Apptron/Wanix VFS persistence is browser-side and shared by shell + IDE.** `static/apptron/index.html` now restores persisted files from IndexedDB (`apptron-wanix-vfs`) before terminal/IDE boot, then snapshots configured paths every 5s. Default persisted paths are `/home`, `/root`, `/public`, `/bin/agent.js`, and `/etc/profile.d/01networking.sh`; override with `localStorage['apptron.persist.paths']` as a JSON string array of absolute guest paths.
- **Apptron/Wanix persistence flush timing:** because users can refresh immediately after edits, persistence now runs every 1s and also flushes on `pagehide`, `beforeunload`, and `visibilitychange=hidden` in `static/apptron/index.html`.
- **Apptron/Wanix persistence safety:** periodic sync is append/update only (non-destructive) to avoid data loss when a transient scan misses paths. Do not delete IndexedDB entries solely because a single snapshot pass did not observe them.
