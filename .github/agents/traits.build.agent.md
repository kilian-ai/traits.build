...
- **Apptron IDE terminal should prefer direct xterm channel.** In `system/src/web/extension.ts`, resolve terminal id from `vm/1/fsys/tmp/.apptron-terminal-id` (fallbacks allowed) and use `web/dom/<id>/data` for read/write; only fall back to `#console/data` when no id is available.
