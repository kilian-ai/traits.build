...
- **Apptron IDE hidden bridge runtime should create xterm but skip `#console/data` bind.** In `static/apptron/index.html`, when `ide_bridge=1` is present, create `web/dom/new/xterm` (to boot shell), skip DOM append and `bind #console/data ...`, and persist terminal id to both `vm/1/fsys/tmp/.apptron-terminal-id` and `localStorage['apptron-terminal-id']` for extension handoff.
