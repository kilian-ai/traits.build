...
- **Terminal PVFS cross-thread sync:** When a REST sentinel dispatch (e.g. `llm.agent` via unknown command) runs on main-thread WASM, `sys.shell` writes files to `localStorage['traits.pvfs']` but the Worker WASM can't see localStorage. The `.finally()` handler after REST dispatch reads localStorage and pushes to the Worker via `backgroundCall('pvfs_load')`. This updates the Worker's `PERSISTENT_VFS` thread-local, making files visible to the next `cli_input`.
