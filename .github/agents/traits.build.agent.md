...
- **kernel.cli unknown-command async dispatch:** fallback now emits a REST sentinel for `llm.agent` (`stream=true`) instead of blocking synchronous execution inside Rust. In WASM terminal this enables progressive output handling and consistent async dispatch path.
