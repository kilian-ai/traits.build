...
- **llm.agent calculator input policy:** for calculator/simple-calculation requests where the user did not provide operands, the agent must not invent demo values. Prefer an interactive terminal script using `prompt()`/stdin first, and only choose canvas when the user explicitly wants a visual calculator or terminal input is not suitable.
- **llm.agent input authenticity policy:** generalize beyond calculators — whenever the user request lacks required runtime inputs, the agent must not invent placeholders, demo constants, CLI args, or synthetic final results. Prefer interactive prompt/stdin collection first, or ask a concise clarification if interaction is impossible.
...
- **kernel.cli unknown-command fallback:** in interactive CLI session mode, unknown commands now attempt a `llm.agent` response (`session_id = "cli-unknown-command"`) before printing the old unknown-command error. This fallback runs in `mode = "full"` with `max_steps = 12` and default toolset (empty tools arg), so the agent can execute multi-step tool-call loops until completion instead of a single round.
...
- **kernel.cli unknown-command config:** fallback seeds/reads `config/unknown_command_agent.json` for `model`, `api_secret`, `tools`, `max_steps`, `retry_max_steps`, and `session_id`.
