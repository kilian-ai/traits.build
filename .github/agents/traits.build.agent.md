...
- **Apptron IDE publishing via Service Worker.** `publish-sw.js` at repo root registers a SW with root scope. The IDE page (`apptron-ide/index.html`) auto-syncs files from `vm/1/fsys/home/root/public` to the SW Cache API every 5 seconds with 2s timeouts per VFS operation. Files in guest `/home/root/public/hello.txt` become accessible at `https://www.traits.build/hello.txt`. Reserved paths (`/`, `/index.html`, `/static/*`, `/traits/*`, `/relay/*`) pass through to the network so the SPA and API are unaffected. The SW supports `publish-file`, `unpublish-file`, `clear-published`, and `list-published` message types. Recursive directory walking (depth 3) handles nested published directories.
