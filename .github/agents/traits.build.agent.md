...
- **Apptron/Wanix VFS persistence uses stock upstream idbfs — not a custom JS layer.** `traits/www/static/apptron/index.html` sets `window.apptron.env = { uuid, name, owner: user_id }` before Wanix boot. This activates the stock `idbfs.New("apptron-rev1")` mounts wired in upstream `boot.go`: `/home/<username>` (always), `/project` (requires `env` + `isOwner`), and `/public` (requires `isOwner`). Setting `owner = user_id` forces `isOwner == true`. The env UUID is stable per env name via `localStorage["apptron.env.uuid.<name>"]`. No custom `apptron-wanix-vfs` IDB layer, no snapshot/restore helpers, no flush hooks — Wanix handles it. Only paths under `/home/<user>`, `/project`, `/public` persist; `/tmp`, `/etc/*`, `/bin/*` are COW memfs over the tarfs bundle and do NOT persist. Remote sync to `{origin}/data/*` will 404 on our setup; this is logged and ignored — local IDB persistence still works.
