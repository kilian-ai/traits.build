...
- SPA `/shell` (`#/shell`) should include a cache-busting `cb` query param when embedding `static/v86/standalone-v86.html`; otherwise the iframe can keep serving stale cached JS after deploy and miss social/UI hotfixes.
...
- `traits/www/static/v86/standalone-v86.html` now has a generic guest command runner (`runGuestCommand`) for GUI→guest CLI integration. Use `beforeRun`/`afterRun` hooks to synchronize browser/UI state with guest files before invoking guest tools like `social.sh`, rather than adding per-button raw `sendCmd(...)` logic.
- When a guest CLI uses `/mnt/vfs` but the browser can only read/write the 9P mirror under `/vfs` (`/mnt/host/vfs` in guest), GUI integration must explicitly mirror state files in both directions. For Social this includes `.nsec`, `.npub`, `.social.tunnel`, and `following/.list`. Without that bridge, UI identity can exist while `social pubkey` still reports "no identity yet".
...
- v86 guest-command queuing must treat Alpine `login:` as a distinct state from a shell. When queued guest work exists on fresh boot, send `root` once at `login:` and do not mark the guest ready until a root-style prompt ending in `# ` appears. A looser prompt detector can mistake echoed shell variables such as `$_tb_cmd ` for a shell prompt and flush commands into the login prompt.
- v86 Social guest-runner commands now persist completion markers at `/mnt/vfs/.guest-runner/<id>.status` (mirrored to `/mnt/host/vfs/.guest-runner/<id>.status`) with values `missing` or `rc:<code>`. Browser polling for these markers must use bounded `fs.read_file` calls (`Promise.race` timeout), because missing files can block indefinitely in some v86 sessions.
- v86 Social guest-runner also writes `/mnt/vfs/.guest-runner/<id>.out` (mirrored to `/mnt/host/vfs/.guest-runner/<id>.out`). Parse `npub1...` / `nsec1...` from this output as a fallback identity hydration path when mirrored `.nsec/.npub` files are stale or missing.
- v86 Social `debug pubkey` should also trigger a direct shell-path check (`SOCIAL_HOME=/mnt/vfs social pubkey`) when shell is ready and parse the live serial tail for `npub1...` / `nsec1...` tokens as a final fallback.
- v86 Social debug now defaults to a step-debug mode (`social_debug` query param, default on) that suppresses automatic probe/rescue churn and emits runner dispatch markers (`seen`, `rc`, `status`) so command-path failures can be diagnosed deterministically.
- v86 Social overlay should run a one-shot guest identity rescue when `social pubkey` succeeds in terminal but UI remains empty: probe fallback roots (`/mnt/host`, `/mnt/host/vfs`, `/root`, `/home/root`, cwd`) for `.nsec/.npub`, then copy into `/mnt/vfs` and `/mnt/host/vfs` before re-running UI hydration.
...
- If relay `/data/*` still receives 403 from both upstream hosts, it returns `204 No Content` with `x-relay-data-fallback: upstream-403-no-content` so hosted IDE boot does not fail with browser `ERR_ABORTED` on blocked sync assets.
...
- **Apptron IDE hidden-shell runtime wait should be bounded.** In `apptron-ide/index.html`, do not await hidden shell `runtime.ready()` indefinitely before handoff. Use a bounded wait (for example 8s) and proceed so the IDE workbench does not stick on loading when shell boot is slow.
- **Apptron IDE must retry wanix port handoff without a short hard timeout.** In `apptron-ide/index.html`, after bounded hidden-shell startup wait, `runtime.createPort()` can still fail for extended cold starts because WANIX boot downloads large assets (roughly 42MB combined for `wanix.wasm` + `sys.tar.gz`). Keep retrying with progress updates instead of failing after 30s, or the workbench can remain stuck on "Loading WASM kernel..." with no `wanix` port delivered.
...
- **Apptron IDE terminal input must use a persistent writable stream.** In `system/src/web/extension.ts`, open terminal data once via `openWritable()`, keep its writer alive for the PTY lifecycle, and serialize `write()` calls; do not use per-keystroke `appendFile()` writes.
- **Apptron IDE hidden bridge runtime should bootstrap xterm and persist terminal id.** In `static/apptron/index.html`, `ide_bridge=1` must still allocate and wire a hidden xterm (`bind #console/data web/dom/<id>/data`, append-child, newline kick) and persist `<id>` to `vm/1/fsys/tmp/.apptron-terminal-id` and `localStorage`. This ensures the shell producer is live before extension attach.
- **Apptron IDE terminal should allocate and fully wire its own xterm channel.** In `system/src/web/extension.ts`, `tryAllocateXterm` must create `web/dom/new/xterm`, bind `task/1/ctl` `#console/data -> web/dom/<id>/data`, append xterm to `web/dom/body/ctl`, kick `web/dom/<id>/data` with `\n`, and then open PTY streams on that exact `web/dom/<id>/data` path. Prefer this deterministic path over stale id discovery.
- **Apptron IDE terminal prompt kick:** after opening `web/dom/<id>/data`, send one initial newline so shells that defer prompt rendering until first input display immediately.
- **Apptron IDE PTY network bootstrap:** after opening terminal writable stream, extension should inject `/tmp/.traits-relay.sh` exports from `localStorage` (`apptron-default-relay`, `apptron-network`, `apptron-network-source`) so active IDE shell sessions inherit WANIX network env even if hidden-shell early writes were dropped.
- **Apptron IDE hidden bridge runtime should write relay script to guest /tmp.** In `static/apptron/index.html`, when `ide_bridge=1` and xterm is created, write `vm/1/fsys/tmp/.traits-relay.sh` with the same relay exports/alias so extension PTY attach can source it directly via `. /tmp/.traits-relay.sh`.
- **Apptron IDE PTY bootstrap must not depend solely on localStorage.** In sandboxed extension-host contexts, `localStorage` may be empty/unavailable; extension bootstrap should fall back to URL/default context (`network`, `relay_url`, `relay`, `worker_url`, default relay) and run one pre-input bootstrap pass in `handleInput` if open-time bootstrap did not complete.
- **Apptron IDE terminal channel fallback should allocate xterm when missing.** In `system/src/web/extension.ts`, if terminal id discovery fails (`.apptron-terminal-id`, localStorage, iframe), attempt `readFile("web/dom/new/xterm")` and use `web/dom/<id>/data`; persist id to `vm/1/fsys/tmp/.apptron-terminal-id` best-effort.
- **Apptron IDE `#console/data` is now a diagnostic override, not the default path.** Keep `?term_path=console` support for debugging, but default PTY attach should target the freshly allocated `web/dom/<id>/data` handle.
- **Apptron IDE terminal auto-network bootstrap:** on PTY open (and once on first input fallback), run `ifconfig eth0 up`, `udhcpc -i eth0 -q -n`, parse DHCP `server` from `/tmp/.udhcpc.log`, then reset default route + `/etc/resolv.conf` to that server IP. This matches Wanix DHCP topology (for example `10.0.0.1`) and avoids stale gateway/DNS values (`10.0.2.2` / public DNS) that break name resolution.
- **Apptron IDE diagnostic toggles:** use `?term_path=console` to force `#console/data` routing in the system extension (legacy path), and `?ide_xterm=0` to disable hidden-iframe xterm creation for bridge A/B testing.
- **Apptron IDE PTY instrumentation markers:** extension terminal attach now emits `[apptron][pty]` stage lines into the VS Code terminal (`path resolved`, `stat(...)`, `openReadable/openWritable`, `initial newline kick`, `first output chunk`, timeout/failure). Use these markers to identify the exact blocking syscall when terminal appears stuck.
- **Apptron IDE hidden iframe must stay render-active.** In `apptron-ide/index.html`, do not use `display:none` for the hidden shell iframe; keep it off-screen (`position: fixed`, negative left, 1x1, opacity 0). `display:none` can suspend xterm/producer lifecycle and yield attach-time channels with no output.
- **Apptron IDE PTY stale-channel recovery:** current debug marker is `pty-stale-channel-recover-20260421-07`. If `web/dom/<id>/data` opens but no first chunk arrives, close that handle and allocate a fresh xterm channel (`web/dom/new/xterm`) before trying `#console/data`. Avoid relying on localStorage-only terminal IDs for initial attach because they can survive iframe restarts and point to dead streams.
- **Apptron IDE failover must not close live PTY writers.** In `system/src/web/extension.ts`, during timeout-based failover do not call `writer.close()` on the primary channel; this can inject EOF to hush and tear down the producer. Use `writer.releaseLock()` before switching channels. Current marker: `pty-no-eof-failover-20260421-08`.
- **Apptron IDE hidden bridge runtime may require screen pipeline enabled.** In `static/apptron/index.html`, when `ide_bridge=1`, boot `WanixRuntime` with `screen: true` (instead of false). Disabled screen can leave `web/dom/<id>/data` / `#console/data` openable but without a live producer. Current marker: `pty-bridge-screen-20260421-09`.
- **Apptron shell should prefer default relay with fallback.** In `static/apptron/index.html`, default network bootstrap should probe `wss://relay.traits.build/linux/tunnel` first, normalize legacy relay URL values (`wss://relay.traits.build/x/net`) to the new tunnel endpoint, preserve explicit `network` query overrides, and use origin-derived `/x/net` as the only non-relay fallback.
- **Apptron shell should proxy hosted /data sync via relay.** In hosted mode, rewrite same-origin `/data/*` fetches to `https://relay.traits.build/data/*` before Wanix boot so browser requests avoid GitHub Pages `/data` 404s and direct cross-origin apptron.dev fetches.
- **Apptron static bundle cache writes may fail and must be non-fatal.** In `static/apptron/index.html`, `getCachedOrFetch` for large artifacts like `bundles/sys.tar.gz` must tolerate `cache.open`, `cache.match`, and `cache.put` failures (for example `TypeError: Failed to fetch`) and continue with direct network fetch + optional gzip decompression so `runtime._bundle` does not reject and boot stall at "Booting Apptron runtime...".
- **Apptron static gzip decompression may fail intermittently and must retry.** `DecompressionStream("gzip")` for `bundles/sys.tar.gz` can throw `TypeError: Failed to fetch` even when fetch succeeds; `static/apptron/index.html` should retry decompression with fresh `fetch(url, { cache: "no-store" })` responses before treating boot as failed.
- **Apptron `cache=clear` must purge bundle cache keys used by runtime boot.** In `static/apptron/index.html`, when `?cache=clear` is present, delete `apptron-bundles` (and diagnostics caches) via `caches.delete(...)` before boot so stale/broken bundle blobs do not persist across reloads.
...
- **Apptron shell should auto-apply DHCP route/DNS on startup.** In `static/apptron/index.html`, the shell bootstrap command must also run `ifconfig eth0 up`, `udhcpc -i eth0 -q -n`, parse DHCP `server` from `/tmp/.udhcpc.log`, then set default route + `/etc/resolv.conf` to that server IP so fresh guest sessions do not start with only loopback.
- **Apptron shell bootstrap retries should cover delayed shell readiness.** `bootstrapRelayInShell` should send relay and network bootstrap commands separately across an extended delay window (0ms, 300ms, 900ms, 1800ms, 3500ms, 6000ms) to survive early xterm write drops.
- **Apptron/Wanix VFS persistence uses stock upstream idbfs — not a custom JS layer.** `traits/www/static/apptron/index.html` sets `window.apptron.user = { username, user_id }` before Wanix boot. This activates the stock `idbfs.New("apptron-rev1")` mount for `/home/<username>` (boot.go:523). The env UUID is stable per env name via `localStorage["apptron.env.uuid.<name>"]`. No custom `apptron-wanix-vfs` IDB layer, no snapshot/restore helpers, no flush hooks — Wanix handles it. Only `/home/<user>` persists by default; `/tmp`, `/etc/*`, `/bin/*` are COW memfs over the tarfs bundle. Remote sync to `{origin}/data/*` is non-fatal for home (logged), so missing backend is acceptable.
- **Apptron shell HOME must be redirected to idbfs mount for persistence.** Upstream Alpine `/etc/passwd` sets root's home to `/root`, which lives on COW memfs and does NOT persist. Idbfs is bound at `/home/<username>` in the Wanix namespace. After xterm creation, `bootstrapRelayInShell` writes a per-session command `mkdir -p /home/<username>; export HOME=/home/<username>; cd /home/<username>; clear` so `~`, `$HOME`, and new files default to the persistent mount. Files written to `/root` will still be lost on reload.
- **Apptron bootstrap rc must be written via fsys, not piped through xterm stdin.** Blasting a ~500-char bootstrap command line through `web/dom/<tid>/data` causes hush to drop leading bytes and can leave the shell in continuation mode (any syntax error like `if ...; then;` with trailing semicolon will stick it there, producing the "shell hangs before prompt" symptom). Instead, write the combined rc script (relay env + DHCP + HOME redirect) to `vm/1/fsys/tmp/.traits-bootstrap.sh` via `runtime.writeFile()` directly, then send only `. /tmp/.traits-bootstrap.sh\n` through the xterm pipe. A 3-attempt retry window (0/800/2000ms) handles early-boot drops; append `clear` on the final shot to wipe any echo noise.
- **Apptron `window.apptron.env` must remain unset by default.** boot.go:597 calls `log.Fatalf("err syncing: ...")` inside the project-fs goroutine whenever `env` is defined. Without an authoritative `/data` backend (the relay is only a local-ack stub), the first remote list returns "file does not exist" and kills the Go runtime before the shell appears. Opt in via `?with_env=1` only after a real `/data` backend is wired. `/project` + `/public` persistence is unavailable in the default configuration.
- **Apptron relay `/data/*` proxy must ack mutating methods with 2xx bodies, not 204.** Wanix httpfs treats `HTTP 204: 204 No Content` as an error in `MkdirAll`, which triggers `log.Fatal` at boot.go:618. The relay returns `200 OK` with body `"ok"` for `PUT/POST/DELETE/PATCH` on `/data/*`, and handles `OPTIONS` preflight locally with `Access-Control-Allow-Methods: GET,POST,PUT,DELETE,PATCH,OPTIONS` and `Access-Control-Allow-Headers: *` (httpfs sends custom `Content-Ownership`, `Content-Mode`, `Content-Modified` headers).
- **Apptron IDE guest networking auto-start via profile.d.** Wanix boots PID 1 as `/bin/sh` (NOT BusyBox init), so `/etc/inittab` is never processed and OpenRC services don't start. Use `/etc/profile.d/01networking.sh` in the Alpine bundle to bring up eth0 + run udhcpc with background retries (tunnel may connect after shell prompt). The "Session IP:" banner is generated by Wanix upstream Go code before profile.d runs — it will be blank but networking works after a few seconds.
- **Wanix idbfs short-write bug: leaks previous tail bytes.** On Wanix idbfs mounts (`/home/<user>`), the shell `> file` redirection writes new bytes at offset 0 but does **not** truncate the tail. If the new payload is shorter than what was there before, stale bytes bleed through. Symptom: JSON/text files mysteriously contain fragments of earlier contents (e.g. `"role":"userem"` = `user` over `system`). **Mitigation:** always `rm -f "$file"` before any `>` redirection in scripts that overwrite files repeatedly. Verify with `printf 'aaaa...' > /tmp/t; printf 'bb' > /tmp/t; cat /tmp/t` — BusyBox `bb` followed by stale `aaa...` reproduces it.
- **BusyBox hush: `echo '...\n...'` does not expand backslashes.** Single-quoted `echo '#!/bin/sh\necho hi' > f.sh` writes the literal two-char sequence `\n`, not a newline. Running the result surfaces as `not found` (the kernel reads `/bin/sh\necho hi` as the interpreter path and ENOENT's). **Use `printf` instead:** `printf '%s\n' '#!/bin/sh' 'echo hi' > f.sh`. LLM-agent system prompts that generate shell scripts must call this out explicitly, otherwise models default to `echo -e`-style backslash escapes that silently corrupt scripts.
- **Apptron IDE publishing via Service Worker.** Publishing is scoped to `/static/apptron-ide/` using `/static/apptron-ide/publish-sw.js`; published files are served from `/static/apptron-ide/public/*`. The IDE page (`apptron-ide/index.html`) auto-syncs files from `vm/1/fsys/home/root/public` every 5 seconds (2s VFS timeouts), keeps per-file content digests, and republishes only changed paths (or all paths when forced). Service-worker responses for published files must include no-store headers (`Cache-Control: no-store`) to prevent stale browser reuse after updates.
- **Apptron IDE publish SW routing guard.** The apptron-ide publishing worker must intercept only paths under `/static/apptron-ide/public/` and must not short-circuit all `/static/*` requests. Cache lookups for published files should use `ignoreSearch: true` so cache-busting query params still resolve to the published artifact.
- **Apptron IDE manual publish trigger command.** In the IDE shell, run `printf '%s\n' "$(date +%s)" > /home/root/.publish-now` to force an immediate publish cycle on the next 5s tick. The trigger is token-based (new content required), and browser console helper `window.apptronPublish.now()` can force publish directly.
- **Apptron IDE trigger token decoding + SW selection.** `runtime.readFile()` for `/home/root/.publish-now` may return bytes (`Uint8Array`/`ArrayBuffer`) rather than string; convert via `TextDecoder` before comparing tokens, otherwise repeated triggers can be ignored (`String(Uint8Array)` collapse). For publish posts, do not rely only on `navigator.serviceWorker.controller`; fall back to `navigator.serviceWorker.ready` registration worker (`active|waiting|installing`) to handle first-load/no-controller states.
- **Apptron IDE publish digest normalization.** `runtime.readFile()` payloads for published files can arrive as object-like byte maps (`{"0": 60, "1": 33, ...}`), not plain strings/Uint8Arrays. Publish auto-sync digesting must normalize numeric-key objects to byte arrays before hashing; otherwise unchanged digest strings like `[object Object]` can cause missed republish on file edits.
- **Container runtime must include `traits/www/static`.** `sys.serve::serve_static` falls back to filesystem path `${TRAITS_DIR}/www/static` (default `./traits/www/static`) for binary and large assets. Docker runtime images that only copy the binary will return 404 for `/static/apptron*` and `/static/apptron-ide*`; copy `traits/www/static` into the image (for example `/traits/traits/www/static`).
- **Apptron IDE SW path for apptron.traits.build.** Browser IDE now registers publishing SW from `/static/apptron-ide/publish-sw.js` (not `/publish-sw.js`) so Fly-hosted `apptron.traits.build` can serve it from static assets. `sys.serve::serve_static` must add `Service-Worker-Allowed: /` for `apptron-ide/publish-sw.js` so the worker can still control root scope and publish `/public/*` to origin root URLs.
- **Apptron IDE publish source path is `/home/root/public`.** The IDE creates and syncs `vm/1/fsys/home/root/public` at boot (idempotent, 2s timeout). Using home-scoped publish source aligns with idbfs persistence semantics for user data and avoids relying on top-level `/public` existence when project-fs is disabled.
- **Apptron IDE seeds a browser-local public viewer at `~/public/viewer.html`.** On boot, the IDE creates `/home/root/public/viewer.html` if it is missing. The viewer does not get VFS access from the publish service worker alone; it opens a hidden same-origin `/static/apptron/index.html` bridge and waits for `window.apptron.ready`, then reads `vm/1/fsys/home/root/public` directly and opens files through `/static/apptron-ide/public/*` URLs. Avoid overwriting `viewer.html` automatically once a user-created file already exists.
- **Apptron public viewer should not auto-refresh.** The seeded `~/public/viewer.html` must avoid periodic polling/auto-refresh loops because they restart stateful pages (games/apps). Keep refresh manual via the button, and avoid resetting the preview iframe when the selected file URL has not changed.
- **Apptron public viewer seed migrations should only touch seeded templates.** Use a versioned marker (for example `apptron-public-viewer:vN`) and migrate only when the existing file contains the seed-marker prefix. Do not overwrite user-authored `~/public/viewer.html` files that do not include the marker prefix.
- **Apptron viewer legacy migration heuristic:** some early seeded `~/public/viewer.html` files predate marker tags. Migrate these only when they match the legacy seeded signature (viewer title + source root constant + old `setInterval(refreshFiles, 5000)` loop), otherwise leave custom files untouched.
- **Apptron viewer migration must decode byte-like `readFile` payloads first.** `runtime.readFile(".../viewer.html")` may return `Uint8Array` or numeric-key objects rather than a JS string; convert to text before checking marker/legacy signatures, or old auto-refresh templates will never migrate.
- **Apptron public viewer must not preview itself in iframe.** When selected file is `viewer.html`, disable iframe preview and show a self-preview-disabled state; otherwise nested viewer-in-viewer recursion can appear as continuous refresh/restart behavior.
- **sys.serve SIGTTOU prevention when backgrounded.** `ensure_repl_tty()` checks foreground process group via `tcgetpgrp`/`getpgrp` before any dup2. Only stdin is dup2'd (never stdout/stderr). `serve_repl_line_mode` opens `/dev/tty` read-only. `serve_repl` skips REPL entirely when stdin is not a terminal.
...
17. **Multi-bridge guest pool corrupts SSH banner (Apr 2026).** Root cause of persistent SFTP `kex_exchange_identification: banner line contains invalid characters` failures: with `tunnel-up.sh` `POOL_SIZE>1` (old default 3), each guest websocat bridge opens its own `tcp:127.0.0.1:22` connection, sshd accepts each separately, and each writes its own `SSH-2.0-...` banner. The Fly Node `tunnel-server` (`addGuest` keeps a Set of WS per port) merged all three banners into a single `guestBuffer[port]`, so the Mac client received `banner1+banner2+banner3` concatenated → KEX-INIT mid-stream → invalid format. Two-part fix: (a) `local/tunnel-up.sh` `POOL_SIZE` default 3 → 1 (HTTP burst workloads can override); (b) `tunnel-server/server.js` `addClient` now collapses any guest pool with size>1 on client attach (closes all entries, drops `guestBuffer`) so the respawn loop creates a single fresh bridge with a single clean banner. Also `local/tunnel-listen.sh` runs websocat in a respawn loop instead of `exec`, so the local listener survives sequential client disconnects.
18. **CF Worker DO close handler clobber (Apr 2026).** In `relay-tunnel/src/index.js` `webSocketClose`, the OLD ws's deferred close handler fires after `_accept` already replaced it, and unconditional `guestWs.delete(port)` clobbered the freshly registered new bridge. Fixed by guarding `if (this.guestWs.get(port) === ws)` before deleting; same for `clientWs`. Also `_accept` now clears `guestBuffer[port]` when replacing an existing guest, mirroring the Fly server's pool-collapse semantics.
19. **Tunnel listener must fork per accept; relay must recycle "used" guest on each new client (Apr 2026).** Two coupled bugs caused SFTP uploads to fail with "socket unexpectedly closed" while `ls`/single-shot ops worked: (a) `local/tunnel-listen.sh` used `websocat -E tcp-l:PORT URL` which is single-shot — it serves one TCP client and exits, so Filezilla's parallel transfer connection had no listener; (b) the relay (both Fly `tunnel-server/server.js` and CF `relay-tunnel/src/index.js`) kept the SAME guest WS alive across multiple client pairings, but each guest WS wraps exactly one `tcp:127.0.0.1:PORT` connection — once the first client paired, sshd already streamed its banner into that pipe and is mid-protocol, so the second client got an empty buffer and protocol garbage. Two-part fix: (a) `local/tunnel-listen.sh` now uses a forking accept loop (socat with `fork EXEC:websocat ...,nofork` if available, else a Python `socket.accept()` loop that `os.fork()` + `dup2(conn, 0/1)` + `execvp("websocat", ["websocat","--binary","-E","-",url])` per connection, falling back to legacy single-shot only with a warning); (b) both relays now track "used" guests/ports and on every new client attach close any guest that has already been paired with a previous client, so the guest-side respawn loop opens a fresh `accept()` and re-emits a clean banner before the new client pairs. Fly tracks via `WeakSet usedGuests`, CF tracks via `Set usedPorts` (Hibernation API safe). Marker added to client-attach close reasons: `guest-recycle-fresh`. **Note**: Fly `tunnel-server` does NOT persist sessions across deploys (in-memory DO equivalent only) — every redeploy invalidates active codes, requiring guest `tunnel-up.sh` re-registration.
20. **Tunnel-up.sh reclaims the same code after relay session loss (Apr 2026).** `local/tunnel-up.sh` now runs a background watchdog that polls `/port/status?code=$CODE` every 20s (override via `TUNNEL_RECLAIM_INTERVAL`); when the relay reports `active:false` (Fly redeploy wiped the in-memory session, CF DO hibernated, network blip, etc.) it re-POSTs `/port/register` with the SAME `{code, ports}` body. Both backends accept `code` in the register body and idempotently restore the session (CF maps code → DO via `idFromName(code)`; Fly stores by string). The websocat respawn loops then succeed on their next iteration without changing `$CODE`, so SSH/SFTP/HTTP clients holding the old pairing code keep working across server restarts. Teardown trap kills `RECLAIM_PID` before unregistering.
22. **Multi-pair byte-flow hardening — retry-on-empty-pool + deferred buffer flush (Apr 2026).** After lesson 21's structural multi-pair refactor, two residual byte-flow bugs surfaced under burst-arrival parallel clients: (a) when a `/port/client` request raced ahead of the guest WS upgrade completing on the relay (small `POOL_SIZE`, simultaneous fanout), `pickFreshGuest` returned undefined and the relay immediately closed with `no guest bridge available` even though guest websockets were arriving milliseconds later. Both backends now retry `pickFreshGuest`/`popFreshGuest` for up to 2.5s (CF: `await new Promise(r => setTimeout(r, 100))` polling inside an `async _accept`; Fly: `setTimeout(retry, 100)` polling in a split `addClient` → `_completeClientPair`), only returning 503/'no guest' after the deadline. (b) Fly's synchronous `ws.send()` flush of pre-pair `guestBuffer` immediately inside the `wss` connection callback could race the just-completed WS handshake's first frames being delivered to the client side websocat, dropping the early banner bytes. Fly now defers the buffer flush via `setImmediate(() => { for (const d of buffered) ws.send(d, {binary:true}); })`, mirroring CF Worker's `setTimeout(() => ..., 0)` pattern. Verified end-to-end: 3 parallel python TCP clients through `tunnel-listen.sh` python forking listener each receive distinct guest banners (`HELLO-5`, `HELLO-6`, `HELLO-7`), with `guest_queue_depth` draining and `active_pairs` reaching 3. Note: zsh's lack of `/dev/tcp/host/port` substitution was an unrelated test-harness false-negative — use bash heredocs or python `socket.connect()` for synthetic parallel-client tests.

23. **Multi-pair queue model for parallel SFTP / FTP / multi-client TCP (Apr 2026).** Earlier "guest recycle on new client" mitigation (lesson 19) only worked for SEQUENTIAL clients on the same port. Filezilla SFTP opens 2+ concurrent SSH connections by default for parallel transfers; the relay's 1:1 client-per-port pairing killed client #1 (`existing.close('replaced')`) AND its paired guest (`'guest-recycle-fresh'`) the moment client #2 arrived, surfacing as `Status: Connecting to 127.0.0.1:2222... Error: Could not read from socket, socket unexpectedly closed` mid-upload. Structural refactor in BOTH backends (`relay-tunnel/src/index.js` CF Worker + `tunnel-server/server.js` Fly Node): replaced single-slot `clientWs: Map<port, ws>` with multi-pair queue model — `guestWs: Map<port, Set<ws>>` (FIFO standby pool of guest bridges), `pairs: Map<ws, ws>` (bidirectional client↔guest map), `guestBuffer: WeakMap<ws, Array>` (per-guest pre-pair buffer, was per-port). Each new client pops a fresh, never-paired guest from the pool via `pickFreshGuest(port)` (skips `pairs.has(ws)` and `proxyCalls.has(ws)`). Existing pairs are NEVER killed by sibling attaches. Close handlers break only the specific pair via `pairs.get(ws)` — sibling pairs on the same port survive. Removed `usedGuests` WeakSet, `pool-collapse`, `pool-collapse-fresh`, `guest-recycle-fresh`, and `replaced` close reasons (all obsoleted by the queue model). Bumped `local/tunnel-up.sh` `POOL_SIZE` default 1 → 4 (override `TUNNEL_BRIDGES_PER_PORT`) so a healthy standby buffer exists for typical interactive use (ssh + 2 sftp transfers + 1 spare); the inner respawn loop refills consumed bridges within ~100ms. Status fields added: `guest_queue_depth`, `active_pairs`. Legacy `guest_ports`/`client_ports`/`paired_ports` kept for tunnel-up.sh watchdog compatibility. **Lesson:** 1:1-per-port pairing is fundamentally incompatible with parallel-capable protocols (SFTP, FTP control+data, HTTP/1.1 keep-alive, parallel rsync) — must use a pool + per-pair routing.
