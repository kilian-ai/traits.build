...
- **Root static mirror for branch-based Pages:** `build.sh` must sync `traits/www/static/` into the repo-root `static/` directory because some deployments serve directly from the root of `main`, where `/static/**` must exist as committed files.
...
- **linux-wasm boot autosync safety:** persistence autosync after shell-ready is now deferred (~1.2s) and runs in boot-safe mode (bounded files/bytes plus cooperative yielding). This mitigates early host-callback starvation/RCU warning escalations during scheduler bring-up. Users can run `persist sync` post-boot for a full restore if boot-safe limits are hit.
- **linux-wasm SMP panic guard:** if early boot logs match irq-work panic signatures (`irq_work_run_list()` / `Aiee, killing interrupt handler!`) before shell-ready, runtime triggers a one-time safe reboot with reduced `maxcpus` (stored in `localStorage['linux-wasm.maxcpus']`) unless user explicitly set `?linux_cpus=`. This is an abstraction-layer mitigation for unstable high-core browser SMP bring-up.
