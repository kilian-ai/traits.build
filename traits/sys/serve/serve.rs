...
/// WebSocket endpoint that spawns a real PTY running a shell with `traits` aliased.
/// Browser xterm.js connects here for a true terminal experience.
async fn ws_terminal(req: HttpRequest, body: web::Payload) -> actix_web::Result<HttpResponse> {
    let (response, mut session, mut msg_stream) = actix_ws::handle(&req, body)?;

    let traits_bin = std::env::current_exe()
        .unwrap_or_else(|_| std::path::PathBuf::from("./target/release/traits"));

    let working_dir = std::env::current_dir()
        .unwrap_or_else(|_| std::path::PathBuf::from("."));

    actix_rt::spawn(async move {
        match pty_spawn(&traits_bin, &working_dir) {
            Ok((master_fd, child_pid)) => {
                // Task: PTY stdout → WebSocket
                let mut ws_tx = session.clone();
                let pty_reader = actix_rt::spawn(async move {
                    loop {
                        let fd = master_fd;
                        let result = tokio::task::spawn_blocking(move || {
                            let mut buf = [0u8; 4096];
                            // Use poll to wait for data with timeout (non-blocking fd)
                            let mut pfd = libc::pollfd { fd, events: libc::POLLIN, revents: 0 };
                            let ret = unsafe { libc::poll(&mut pfd, 1, 100) }; // 100ms timeout
                            if ret > 0 && (pfd.revents & libc::POLLIN) != 0 {
                                let n = unsafe { libc::read(fd, buf.as_mut_ptr() as *mut _, buf.len()) };
                                if n > 0 { Ok(Some(buf[..n as usize].to_vec())) }
                                else { Err("eof") }
                            } else if ret == 0 {
                        ...
                        match result {
                            Ok(Ok(Some(data))) => {
                                if ws_tx.binary(data).await.is_err() { break; }
                            }
                            Ok(Ok(None)) => continue, // poll timeout
                ...
                while let Some(Ok(msg)) = msg_stream.next().await {
                    match msg {
                        ...
                        actix_ws::Message::Ping(data) => { let _ = session.pong(&data).await; }
            ...
            Err(e) => {
                let _ = session.text(format!("\x1b[31mFailed to spawn terminal: {}\x1b[0m\r\n", e)).await;
...
/// Spawn a PTY with a shell that has the traits binary aliased.
/// Returns (master_fd, child_pid).
fn pty_spawn(traits_bin: &std::path::Path, working_dir: &std::path::Path) -> Result<(i32, i32), String> {
    use std::ffi::CString;
    ...
    unsafe {
        ...
        if pid == 0 {
            ...
            if let Ok(dir_c) = CString::new(working_dir.to_string_lossy().as_bytes()) {
            ...
            // Write init script that aliases traits/t and sets a clean prompt
            let init_path = format!("/tmp/.traits_ws_init_{}", libc::getpid());
            let init_script = format!(
                "alias traits='{}'\nalias t='{}'\nexport PS1='\\[\\033[36m\\]traits\\[\\033[0m\\]> '\n",
                traits_bin.display(), traits_bin.display(),
            );

            let shell = CString::new("/bin/bash").unwrap();
            let arg0 = CString::new("bash").unwrap();
            let norc = CString::new("--norc").unwrap();

            if let Ok(init_c) = CString::new(init_path.as_bytes()) {
                let mode = CString::new("w").unwrap();
                let fp = libc::fopen(init_c.as_ptr(), mode.as_ptr());
                if !fp.is_null() {
                    let sc = CString::new(init_script.as_bytes()).unwrap();
                    libc::fputs(sc.as_ptr(), fp);
                    libc::fclose(fp);
                }

                let init_flag = CString::new("--init-file").unwrap();
                let argv: [*const libc::c_char; 5] = [
                    arg0.as_ptr(), norc.as_ptr(), init_flag.as_ptr(), init_c.as_ptr(), std::ptr::null(),
                ];
                libc::execvp(shell.as_ptr(), argv.as_ptr());
            }

            // Fallback
            let argv: [*const libc::c_char; 3] = [arg0.as_ptr(), norc.as_ptr(), std::ptr::null()];
            libc::execvp(shell.as_ptr(), argv.as_ptr());
            libc::_exit(1);
        }
